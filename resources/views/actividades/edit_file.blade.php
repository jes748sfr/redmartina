<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar archivos de la actividad') }}
        </h2>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                            ...
                            <li class="flex flex-wrap items-center justify-between gap-4 border-b pb-2">
                                ...
                                @if(in_array(strtolower($extension), $extensionesImagen))
                                    ...
                                    <div class="w-20 h-20 flex items-center justify-center rounded-md border overflow-hidden">
                                        <a href="{{ asset('storage/documentacion_actividades/'.$documento->archivo) }}" data-lightbox="documento">
                                            <img src="{{ asset('storage/documentacion_actividades/'.$documento->archivo) }}" alt="Documento" class="max-w-full h-auto object-cover">
                                        </a>
                                    </div>
                                @else
                    ...
                    <form id="uploadForm" action="{{ route('documentacion_actividad.store2') }}" method="POST" enctype="multipart/form-data" class="mt-4">
                        @csrf
                        <input type="hidden" name="id_actividades" value="{{ $actividad->id }}">
    ...
    document.querySelectorAll(".archivoActualizar").forEach(button => {
        button.addEventListener("click", function () {
            ...
            input.addEventListener("change", function () {
                ...
                //let url = `/actividades/subir_archivos/update/${id}`;
                let url = "{{ route('documentacion_actividad.update', ['id' => '__ID__']) }}";
                let i = id;
    ...
    document.querySelectorAll(".archivoEliminar").forEach(button => {
        button.addEventListener("click", function () {
            let id = this.getAttribute("data-id");
            //let url = `/actividades/subir_archivos/delete/${id}`;
            let url = "{{ route('documentacion_actividad.delete', ['id' => '__ID__']) }}";
