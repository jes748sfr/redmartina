<x-app-layout>
    <x-slot name="header">
        ...
        <link rel="stylesheet" href="{{ asset('css/index_logeado.css') }}">
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('buscar_martiana') }}" class="mb-6" autocomplete="off">
                        @csrf
                        <div class="flex items-center">
                            
                            @if(isset($query))
                                <a href="{{ route('martianas.auth') }}" id="btn_regresar" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center bg-blue-500 hover:bg-blue-600 text-white font-bold mr-4 rounded-full transition-all duration-300 items-center justify-center">
                            ...
                            <input 
                                ...
                                name="keyword" 
                                value="{{ $query ?? '' }}" 
                                placeholder="Buscar actividades martianas..." 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                    @forelse($martianas as $martiana)
                        <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col justify-between">
                            <div>
                                <a href="{{ route('editar_Martiana', ['id' => $martiana->id]) }}" title="{{ $martiana->titulo }}">
                                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 truncate min-h-[48px]">
                                        {{ Str::limit($martiana->titulo, 50, '...') }}
                                    </h5>
                                </a>
                                <p class="mb-3 font-normal text-gray-700 text-justify">{!! $martiana->cuerpo_truncado !!}</p>
                            </div>
                            <div class="mt-auto flex gap-2">
                                <a href="{{ route('editar_Martiana', ['id' => $martiana->id]) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-600 rounded-full hover:bg-blue-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                                    </svg>                                      
                                </a>
                                <form action="{{ route('martianas.delete', $martiana->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <div class="col-span-4 text-center">
                            <p class="text-gray-500">No se encontraron actividades martianas.</p>
                        </div>
                    @endforelse
                </div>
                <div class="mt-6">
                    @if(isset($query))
                        {{ $martianas->appends(['keyword' => $query])->links() }}
                    @else
                        {{ $martianas->links() }}
                    @endif 
                </div> 
    ...
    <div class="fixed bottom-4 right-4">
        <a href="{{ route('crear_Martiana') }}" id="btn_agregar" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 overflow-hidden">
