<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('buscar_convocatoria') }}" class="mb-6" autocomplete="off">
                        @csrf
                        <div class="flex items-center">
                            
                            @if(isset($query))
                                <a href="{{ route('convocatorias.auth') }}" id="btn_regresar" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center bg-blue-500 hover:bg-blue-600 text-white font-bold mr-4 rounded-full transition-all duration-300 items-center justify-center">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                    @forelse($convocatorias as $convocatoria)
                        <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col justify-between">
                            <div>
                                <a href="{{ route('editar_Convocatoria', ['id' => $convocatoria->id]) }}" title="{{ $convocatoria->titulo }}">
                            ...
                            <div class="mt-auto flex gap-2">
                                ...
                                <form action="{{ route('convocatorias.delete', $convocatoria->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="fixed bottom-4 right-4">
        <a href="{{ route('crear_Convocatoria') }}" id="btn_agregar" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 overflow-hidden">
